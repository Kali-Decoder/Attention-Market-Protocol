...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden relative">
      <header className="shrink-0 h-16 bg-[#18181b] border-b border-[#2f2f35] flex items-center px-6 md:px-10 z-20">
        <div className="flex-1 flex items-center">
          <Link to="/" className="flex items-center space-x-2 mr-8 md:mr-20">
        ...
        <div className="flex items-center space-x-3 ml-8">
          {!isConnected ? (
            <button
              ...
              className="bg-black text-white px-6 py-2 rounded-lg font-medium hover:bg-white hover:text-black transition-all duration-200"
            >
              Sign In
            </button>
          ) : (
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <button
          ...
          className={`fixed z-50 bg-[#232327] border border-[#2f2f35] border-r-0 rounded-r-lg p-2 hover:bg-[#2f2f35] transition-all duration-300 top-1/2 -translate-y-1/2 ${isSidebarExpanded ? 'left-64' : 'left-0'}`}
        ...
        <main className="flex-1 min-h-0 min-w-0 overflow-y-auto overscroll-y-contain bg-black text-white">
          {children}
        </main>
      </div>
